...
#:mu:   =  gbar( ringV(:u:,:v:,:w:) * :rho: ) * 1.0e-4 + #wolf#
:mu:   =  gbar( ring(:S:  ) * :rho: ) * 1.0e-4 + #wolf#
# Tau
[:ux:,:uy:,:tz:] = grad(:u:)
[:vx:,:vy:,:tz:] = grad(:v:)
...
:umag: = sqrt( :u:*:u: + :v:*:v: )
"""
eom = eom.replace('u0',str(u0)).replace('p0',str(p0)).replace('rho0',str(rho0)).replace('#wolf#',str(mu0))
